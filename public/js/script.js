...
function addPeople() {
  ...
  fetch('/users', {
    ...
    body: JSON.stringify({ peopleData }), // Send the people data to your backend
  })
...
function howManyHours() {
  const htmlContainer = document.querySelector('.js-howManyHours');
  ...
  peopleData.forEach((person, index) => {
    const personHtml = `
      <div class="person">
        ...
        <input placeholder="Имя" class="inputForName" value="${person.name}" oninput="updateName(${index}, this.value)">
        <input placeholder="Время" class="inputForTime" value="${person.time}" oninput="updateTime(${index}, this.value)">
        <input placeholder="Цена" class="inputForPrice" value="${person.price}" oninput="updatePrice(${index}, this.value)">
        <button class="buttonSave" onclick="savePerson(${index})">Сохранить</button>
      </div>
    `;

    htmlContainer.innerHTML += personHtml;
  });
}

function updateName(index, name) {
  peopleData[index].name = name;
}

function updateTime(index, time) {
  peopleData[index].time = parseFloat(time) || 0;
}

function updatePrice(index, price) {
  peopleData[index].price = parseFloat(price) || 0;
}

function savePerson(index) {
  const person = peopleData[index];

  // Send the data to the server for saving to the database
  fetch('/savePerson', {
    method: 'POST',
    ...
    .then((response) => {
      if (response.ok) {
        ...
        alert(`Person ${index + 1} data saved.`);
      } else {
        // Handle errors
        console.error(`Failed to save person ${index + 1} data.`);
      }
    })
    .catch((error) => {
...
function playSound() {
  const audio = new Audio('sound/sound-effect.wav');
  audio.play();
}
...
function addLoggedInCSS() {
  ...
  loggedInCSS.href = 'css/front.css'; // replace 'front.css' with the CSS file for logged-in state
